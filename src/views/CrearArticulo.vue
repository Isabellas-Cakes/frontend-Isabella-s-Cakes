<template>
  <v-container>
    <v-row align="center">
      ...
      <v-col>
        <h2 class="text-h5 font-weight-bold">Crear artículo</h2>
      </v-col>
    </v-row>
    ...
    <v-form @submit.prevent="guardarArticulo">
      <v-text-field
      ...
      <p class="text-caption mt-2">
        Podrás agregar los ingredientes de este artículo una vez lo hayas
        guardado.
      </p>
...
const articulo = ref({
  ...
  costo: null,
  disponible: true,
  id_categoria: parseInt(idCategoria), // Asegurar que sea número
});
...
const validarFormulario = () => {
  ...
  if (!articulo.value.id_categoria) {
    showSnackbar("La categoría es requerida", "error");
    isValid = false;
  }
...
const guardarArticulo = async () => {
  ...
  try {
    ...
    formData.append("estado", articulo.value.disponible); // ✅ Cambiado de 'disponible' a 'estado'
    formData.append("id_categoria", articulo.value.id_categoria);
    formData.append("stock_actual", 0); // ✅ Agregado campo requerido
    ...
    console.log("- estado:", articulo.value.disponible);
    console.log("- id_categoria:", articulo.value.id_categoria);
    console.log("- stock_actual:", 0);
    console.log("- imagen:", imagenFile.value ? "Sí" : "No");

    const response = await axios.post(
  "https://api-isabella-s-cakes.onrender.com/api/articulos",
      formData,
      {
        headers: { 
    ...
    showSnackbar("¡Artículo creado exitosamente!", "success");
    
    // Esperar un momento antes de redirigir para que el usuario vea el mensaje
    setTimeout(() => {
      router.push(`/categorias/${idCategoria}/articulos`);
    }, 1500);
